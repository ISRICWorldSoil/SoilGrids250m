...
load(".RData")
profs <- read.csv("mangrove_soc_database_v10_sites.csv", skip=1)
#str(profs)
profs.f <- rename(profs, c("Site.name"="SOURCEID", "Longitude_Adjusted"="LONWGS84", "Latitude_Adjusted"="LATWGS84"))
...
hors <- read.csv("mangrove_soc_database_v10_horizons.csv", skip=1)
#str(hors)
