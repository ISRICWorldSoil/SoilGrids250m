## Reading and writing of NASIS pedon data [http://soils.usda.gov/technical/nasis/]
## tom.hengl@isric.org
...
proj4string(NASISgg.pnts)
## 310,690 points
#str(NASISgg.pnts@data)
#summary(NASISgg.pnts$gg)
TAXOUSDA.NASIS <- plyr::rename(NASISgg.pnts@data[,c("upedonid","xwgs84","ywgs84","grtgrp_nm")], replace=c("upedonid"="SOURCEID", "xwgs84"="LONWGS84", "ywgs84"="LATWGS84", "grtgrp_nm"="TAXNUSDA"))
TAXOUSDA.NASIS$SOURCEDB = "NASIS"
GreatGroups = read.csv("TAXOUSDA_GreatGroups_complete.csv", stringsAsFactors = FALSE)
str(GreatGroups)
GreatGroups$TAXNUSDA = toupper(paste(GreatGroups$Great_Group))
TAXOUSDA.NASIS$TAXOUSDA <- plyr::join(TAXOUSDA.NASIS, GreatGroups, match="first")$Suborder
str(TAXOUSDA.NASIS)
coordinates(TAXOUSDA.NASIS) <- ~ LONWGS84 + LATWGS84
proj4string(TAXOUSDA.NASIS) <- CRS("+proj=longlat +datum=WGS84")
#plotKML(TAXOUSDA.NASIS["TAXOUSDA"])
save(TAXOUSDA.NASIS, file="TAXOUSDA.NASIS.rda")
