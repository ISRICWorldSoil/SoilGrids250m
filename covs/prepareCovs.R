prepareCovsSoilGrids250m <- function(nms=c("DEM","MOD13Q1","MCD43A4","MOD11A2","MOD10A2","PREm","EcoTapestry","Other"), s.zone, s.lst, close.gap=TRUE){
  ...
  if(any(nms %in% "MOD11A2")){  
    for(k in 1:length(LSTD.out.lst)){
      for(i in s.zone){
        if(missing(s.lst)){ s.lst <- 1:length(equi7t3[[i]]) }
        if(LSTD.out.lst[k]=="TSDMOD3"){ 
          sapply(s.lst, function(x){tile.tif(t=equi7t3[[i]][x,], input=LSTD.lst[k], nm=LSTD.out.lst[k], resample.type="cubicspline", ot="Int16", nodata="-32767", srcnodata="-32768", Z_SCALE=10, zmin=0, close.gap=close.gap)})
        } else {
        ...
        if(LSTN.out.lst[k]=="TSNMOD3"){ 
          sapply(s.lst, function(x){tile.tif(t=equi7t3[[i]][x,], input=LSTN.lst[k], nm=LSTN.out.lst[k], resample.type="cubicspline", ot="Int16", nodata="-32767", Z_SCALE=10, srcnodata="-32768", zmin=0, close.gap=close.gap)})
        } else {
  ...
  if(any(nms %in% "MOD10A2")){
    for(k in 1:length(SNW.out.lst)){
      for(i in s.zone){
        if(missing(s.lst)){ s.lst <- 1:length(equi7t3[[i]]) }
        sapply(s.lst, function(x){tile.tif(t=equi7t3[[i]][x,], input=SNW.lst[k], nm=SNW.out.lst[k], resample.type="bilinear", ot="Int16", nodata="-32767", zmin=0, close.gap=close.gap)})
      }
    }
  }
  ...
  if(any(nms %in% "PREm")){
    for(k in 1:length(PREm.out.lst)){
      for(i in s.zone){
        if(missing(s.lst)){ s.lst <- 1:length(equi7t3[[i]]) }
        sapply(s.lst, function(x){tile.tif(t=equi7t3[[i]][x,], input=PREm.lst[k], nm=PREm.out.lst[k], resample.type="bilinear", ot="Int16", nodata="-32768", Z_SCALE=0.05, srcnodata=65535, dstnodata=65535, zmin=0, close.gap=close.gap)})
      }
    }
  }
  ...
  if(any(nms %in% "Other")){
    for(i in s.zone){ 
      if(missing(s.lst)){ s.lst <- 1:length(equi7t3[[i]]) }
      sapply(s.lst, function(x){tile.tif(t=equi7t3[[i]][x,], input="/data/Groundwater/GTDHYS3a.tif", nm="GTDHYS3", resample.type="cubicspline", ot="Int16", nodata="-32767", srcnodata=-99999, zmin=0, close.gap=close.gap)})
    }
  }
