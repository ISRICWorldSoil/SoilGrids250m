## Tile and close gaps - SoilGrids250m
## by: Tom.Hengl@isric.org
## Uses only a single core! this way it can be parallelized
...
tile.sdat <- function(input, t, tr=250, t_srs = proj4string(t), resample.type, Z_SCALE, srcnodata, dstnodata, TYPE){
  ...
  te <- as.vector(bbox(t))
  ## Byte/Int16/UInt16
  system(paste0(gdalwarp, ' ', input, ' ', tmp, ' -t_srs \"', t_srs, '\" -tr ', tr, ' ', tr, ' -r \"', resample.type, '\" -srcnodata \"', srcnodata ,'\" -dstnodata \"', dstnodata, '\" -te ', paste(te, collapse=" "), ' -of \"SAGA\"')) 
  if(!Z_SCALE==1){
...
close.gaps <- function(inputTile, maskTile, outTile, ot, nodata, a_srs, zmin){
...
tile.tif <- function(t, input, nm, resample.type="near", ot, nodata, Z_SCALE=1, srcnodata=-32767, dstnodata=srcnodata, TYPE=7, zmin=srcnodata, close.gap=TRUE){
   ns <- strsplit(paste(t$SHORTNAME), " ")[[1]][2]
   maskTile <- paste0("./", ns, "_", t$TILE, "/LMK_", ns, "_", t$TILE, ".sgrd")
   outTile <- set.file.extension(gsub("LMK", nm, maskTile), ".tif")
   if(!file.exists(outTile)){
     ...
     if(close.gap==TRUE){
       close.gaps(inputTile=outn, maskTile=maskTile, outTile=outTile, ot=ot, nodata=nodata, a_srs=proj4string(t), zmin=zmin)
     } else {
