...
=======
## 3397
>>>>>>> 85569e3559e6a87deb60be15907ccb3bbd1e2d79
...
## Test:
predict_factor_tile(i=3483, mRF, varn, levs, in.path="/data/NASIS/covs100m", out.path="/data/NASIS/predicted100m", PMTGSS7.leg, DRNGSS7.leg)


## Split models otherwise too large in size:
num_splits=30
mRF_final <- split_rf(mRF, num_splits)
for(j in 1:length(mRF_final)){
  gm = mRF_final[[j]]
  saveRDS.gz(gm, file=paste0("mrfX_", varn, "_", j,".rds"))
}
rm(mRF); rm(mRF_final)
>>>>>>> 85569e3559e6a87deb60be15907ccb3bbd1e2d79
gc(); gc()
...
for(j in 1:num_splits){
  gm = readRDS.gz(paste0("mrfX_", varn,"_", j,".rds"))
  cpus = unclass(round((256-50)/(3.5*(object.size(gm)/1e9))))
