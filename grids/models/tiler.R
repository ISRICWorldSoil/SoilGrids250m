...
tile.sdat <- function(input, t, tr=250, t_srs = proj4string(t), resample.type, Z_SCALE, srcnodata, dstnodata, TYPE){
  ...
  if(!Z_SCALE==1){
     ...
     system(paste0(saga_cmd, ' -c=', cpus, ' grid_calculus 1 -GRIDS ', set.file.extension(tmp, ".sgrd"), ' -FORMULA g1*', Z_SCALE, ' -INTERPOLATION 0 -TYPE ', TYPE, ' -RESULT ', set.file.extension(tmp, ".sgrd")))
  }
  return(tmp)
}

## generic function to fill all gaps:
close.gaps <- function(inputTile, maskTile, outTile, ot, nodata, nodata_out, a_srs, zmin, method, cpus=1, fix.zmin=TRUE){
  if(pkgmaker::file_extension(inputTile)=="tif"){ 
    system(paste0(gdal_translate, ' ', inputTile, ' ', gsub(".tif", ".sdat", inputTile), ' -of \"SAGA\" -ot \"Int16\" -a_nodata \"', nodata, '\"'))
    inputTile = gsub(".tif", ".sdat", inputTile)
  }
  tmp <- RSAGA::set.file.extension(tempfile(), ".sgrd")
  if(fix.zmin==TRUE){
    r <- raster(inputTile)
    zmin.count <- getValues(r<zmin)
    s.zmin.count <- sum(zmin.count,na.rm=TRUE)
    na.count <- getValues(is.na(r))
    na.count.mask <- sum(getValues(is.na(raster(RSAGA::set.file.extension(maskTile, ".sdat"))))&(zmin.count|na.count), na.rm=TRUE) 
    ...
    if( na.count.mask>0 & !s.zmin.count==ncell(r) & na.count.mask<0.95*ncell(r) ){
      if(s.zmin.count>0){
        if(zmin>=0){
          system(paste0(saga_cmd, ' -c=', cpus, ' grid_calculus 1 -GRIDS ', RSAGA::set.file.extension(inputTile, ".sgrd"), ' -FORMULA=\"ifelse(g1<', zmin, ',-99999,g1)\" -INTERPOLATION 0 -RESULT ', RSAGA::set.file.extension(inputTile, ".sgrd")))
        }
      }
    }
  }
  if(method=="stepwise"){
    ...
    system(paste0(saga_cmd, ' -c=', cpus, ' grid_tools 29 -INPUT ', RSAGA::set.file.extension(inputTile, ".sgrd"), ' -MASK ', maskTile, ' -RESULT ', tmp, ' -RESAMPLING=1 -PYRAMIDS=1'))
  } else {
    ## http://saga-gis.org/saga_module_doc/2.2.0/grid_tools_7.html 
    system(paste0(saga_cmd, ' -c=', cpus, ' grid_tools 7 -INPUT ', RSAGA::set.file.extension(inputTile, ".sgrd"), ' -MASK ', maskTile, ' -RESULT ', tmp))
  }
  system(paste0(gdal_translate, ' ', RSAGA::set.file.extension(tmp, ".sdat"), ' ', outTile, ' -ot \"', ot, '\" -a_nodata \"', nodata_out, '\" -a_srs \"', a_srs, '\" -co \"COMPRESS=DEFLATE\"'))
  unlink(RSAGA::set.file.extension(tmp, ".sdat"))
  unlink(RSAGA::set.file.extension(inputTile, ".sdat"))
}
