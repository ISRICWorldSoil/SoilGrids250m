...
hist(soilwell$BDTICM, main = "", xlim =  quantile(soilwell$BDTICM, probs = c(0, 0.99), na.rm = T), breaks =1000 , xlab = "Absolute Depth to bedrock (cm)")
hist(log1p(soilwell$BDTICM), main = "", xlab = "Log-transformed absolute depth to bedrock (cm)")
dev.off()
...
hist(soilwell2[soilwell2$SOURCEDB!= "Wells",]$BDRICM, main = "", xlab = "Censored depth to bedrock from soils(cm)", breaks= 20  )
hist(soilwell2[soilwell2$SOURCEDB == "Wells",]$BDRICM, main = "", xlab = "Censored depth to bedrock from wells(cm)", breaks= 20 )
dev.off()
par(mar = c(5.1,4.1,4.1,2.1), mfcol=c(1,1))
