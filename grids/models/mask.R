...
sapply(1:nrow(zp.lst), function(x){unzip(zipfile, files=zp.lst$Name[x], overwrite=FALSE, exdir=paste0("/data/covs/", dir.lst[x]))})
...
## clean-up:
#rm.lst <- list.files(pattern=glob2rx("*.tif$"), full.names=TRUE, recursive=TRUE)
