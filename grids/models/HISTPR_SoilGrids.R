...
sfLibrary(raster)
sfLibrary(rgdal)
out <- sfClusterApplyLB(pr.dir, function(i){histosol.prob(i, in.path="/data/predicted1km", fao.lst, usda.lst)})
sfStop()
...
sfLibrary(rgdal)
sfLibrary(GSIF)
out <- sfClusterApplyLB(pr.dir, function(i){wrapper.OCSTHA(i, in.path="/data/predicted1km")})
sfStop()
