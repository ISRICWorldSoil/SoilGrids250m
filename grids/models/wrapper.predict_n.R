...
wrapper.predict_n <- function(i, gm_path1, gm_path2, sd=c(2.5, 7.5, 22.5, 45, 80, 150), varn, in.path, out.path, z.min, z.max, gm1.w, gm2.w, type="Int16", mvFlag=-32768){ ## , nthreads 
      ...
      if(any(!file.exists(out.c))){
        for(x in 1:length(varn)){
          if(!file.exists(out.c[x])){
            ## predict:
            gm1 <- h2o.loadModel(gm_path1[[varn[x]]], h2o.getConnection())
            if(missing(gm1.w)){ gm1.w = gm1@model$training_metrics@metrics$r2 }
            gm2 <- h2o.loadModel(gm_path2[[varn[x]]], h2o.getConnection())
